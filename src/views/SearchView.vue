<script setup>
    ...
    const performSearch = async () => {
      ...
      try {
        // 1. POST ARAMASI (Başlık, İçerik)
        // Etiket araması için Supabase'de 'cs' (contains) kullanılır ama text search için
        // şimdilik basit 'or' mantığıyla başlık ve içeriğe bakalım.
        ...
        // 2. PROFİL ARAMASI (İsim, Uzmanlık, Bio)
        ...
        <div class="max-w-7xl mx-auto px-4 py-8">
          ...
          <div class="border-b border-gray-200 mb-8">
            <nav class="-mb-px flex space-x-8">
              ...
              >
                Kişiler / Uzmanlar ({{ profiles.length }})
              </button>
            </nav>
          </div>
  ...
  <PostCard v-for="post in posts" :key="post.id" :post="post" />
          ...
          <div v-else-if="activeTab === 'profiles'">
            <div v-if="profiles.length === 0" class="text-center py-20 text-gray-500">
              Aradığınız kelimeye uygun kişi bulunamadı.
            </div>
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <RouterLink 
                ...
                class="bg-white p-6 rounded-lg shadow-sm border hover:shadow-md transition flex items-center space-x-4"
              >
                <div class="w-16 h-16 rounded-full bg-primary-100 flex items-center justify-center text-primary-600 text-xl font-bold flex-shrink-0">
                  {{ profile.full_name?.[0] }}
                </div>
                <div>
                  <h3 class="font-bold text-gray-900">{{ profile.full_name }}</h3>
