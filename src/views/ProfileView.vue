...
const followLoading = ref(false) // Buton yükleniyor mu?
...
const fetchData = async () => {
  try {
    ...
    // 3. Takip Kontrolü (Giriş yapmışsa ve kendi profili değilse)
...
const toggleFollow = async () => {
  ...
  try {
    ...
    if (isFollowing.value) {
      ...
      // Sayacı anlık düşür (Trigger arkadan güncelleyecek ama biz kullanıcıya hızlı gösterelim)
...
<template>
  <DefaultLayout>
    ...
    <div v-else-if="!profile" class="text-center py-20 text-gray-500">
      Kullanıcı bulunamadı veya profil gizli.
    </div>
      ...
      <div class="bg-white border-b mb-8">
        <div class="max-w-4xl mx-auto px-4 py-10 text-center">
          ...
          <div class="mb-4 relative inline-block">
            ...
            >
            <div v-else class="w-32 h-32 mx-auto bg-primary-100 text-primary-600 rounded-full flex items-center justify-center text-4xl font-bold border-4 border-white shadow-lg">
              {{ profile.full_name?.[0] }}
            </div>
          ...
          <div class="mb-8 flex flex-wrap justify-center gap-3">
            ...
            <!-- 1. TAKİP BUTONU -->
            <!-- Sadece giriş yapmış ve başkasının profili ise göster -->
            <button 
