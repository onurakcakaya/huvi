...
// Meslek Listesi (Veritabanından gelecek)
...
onMounted(async () => {
  ...
  try {
    ...
    if (data) professionList.value = data

  } catch (error) {
    console.error('Meslek listesi yüklenemedi:', error.message)
  }
})
      ...
      <div class="p-8">
        ...
        <form @submit.prevent="updatePublisherProfile" class="space-y-8">
          ...
          <div class="space-y-6">
            ...
            <!-- GÜNCELLENEN KISIM: INPUT YERİNE SELECT GELDİ -->
            <div>
              <label class="block text-sm font-bold text-gray-900 mb-2">Uzmanlık Alanı / Unvan</label>
              <select 
                v-model="profession" 
                class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 shadow-sm bg-white"
              >
                <option value="" disabled>Lütfen bir uzmanlık seçiniz...</option>
            ...
            <div>
              ...
              <textarea v-model="bio" rows="4" placeholder="Deneyiminizden, eğitiminizden ve tarzınızdan bahsedin..." class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 shadow-sm"></textarea>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-900 mb-2">Website / Sosyal Medya</label>
              <input v-model="website" type="text" placeholder="https://instagram.com/kullaniciadi" class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 shadow-sm">
            </div>
          </div>
